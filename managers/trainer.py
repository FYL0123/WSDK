...
class Trainer(ABC):
    def __init__(self, args):
        ...
        self.save_root = Path('./ckpt/{}'.format(self.config['checkpoint_name']))
        ...
        for epoch in range(self.config['epoch']):
            ...
            for idx, inputs in enumerate(bar):
                ...
                if 'backbone' in self.all_optimized_modules:
                    ...
                    #self.logger.info(grad_message)
                    grad_message = 'grad backbone firstconv mean {:.6f} max{:.6f}'.format(self.model.backbone.block1.conv1.weight.grad.mean().item(),
                        self.model.backbone.block1.conv1.weight.grad.max().item())
                    self.logger.info(grad_message)
                ...
                #    epoch_path_save_ = self.save_root / '{:>03d}'.format(epoch) / '{:>03d}'.format(idx)
                #    epoch_path_save_.makedirs_p()
                #    self.model.save_checkpoint(epoch_path_save_)
                    self.model.save_checkpoint(epoch_path)
    ...
    def val_and_vis(self, batch_path_list, idx):
        ...
        for i, cur_path in enumerate(batch_path_list):
            ...
            color = (0,255,0)
            for kp1 in cur_kps1:
                cv2.circle(comb_img_kps, (int(kp1[0]),int(kp1[1])), radius=2, color=color, thickness=-1)
            for kp2 in cur_kps2:
            ...
            for kp1, kp2 in zip(cur_matchkp1, cur_matchkp2):
                cv2.circle(comb_img_kps_m, (int(kp1[0]),int(kp1[1])), radius=2, color=color, thickness=-1)
            ...
            for kp1, kp2, color in zip(cur_matchkp1_less, cur_matchkp2_less, match_color_less):
                ...
                cv2.line(comb_img_m_less, (int(kp1[0]),int(kp1[1])), kp2_comb, color, thickness=2)
                cv2.circle(comb_img_m_less, (int(kp1[0]),int(kp1[1])), radius=2, color=(0,255,0), thickness=-1)
            ...
            for kp1, kp2, color in zip(cur_matchkp1, cur_matchkp2, match_color):
                ...
                kp2_comb = (int(kp2[0]+w+mid_pad), int(kp2[1]))
                color = (int(color[0]), int(color[1]), int(color[2]))
                cv2.line(comb_img_m, (int(kp1[0]),int(kp1[1])), kp2_comb, color, thickness=2)
                cv2.circle(comb_img_m, (int(kp1[0]),int(kp1[1])), radius=2, color=(0,255,0), thickness=-1)
