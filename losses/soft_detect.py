...
class SoftDetect(LightningModule):
    def __init__(self, radius=2, top_k=3000, scores_th=0.3, n_limit=5000):
        """
        Args:
    ...
    def detect_keypoints(self, scores_map, normalized_coordinates=True):
        ...
        for b_idx in range(b):
            max = scores_map[b_idx].view(-1).max().item()
            scores_map_norm = (scores_map[b_idx] - (torch.full((1, 480, 640), max).to('cuda')))
            softmax_score = torch.nn.functional.softmax(scores_map_norm.view(-1), dim=0).view(1, 480, 640)
            ...
            #scoreprob = x_exp / x_exp.sum(dim=1)
            ...
            kpt_prob = torch.nn.functional.grid_sample(softmax_score.unsqueeze(0), keypoints_xy.view(1, 1, -1, 2),
                                                       mode='bilinear', align_corners=True)[0, 0, 0, :]
            #kpt_prob = ((kptscore.reshape(-1, 1) - max_v) / self.temperature).exp().sum(dim=1)
            #kpt_prob = kpt_prob / x_exp.sum(dim=1)
            # CxN

            keypoints.append(keypoints_xy)
            scoredispersitys.append(scoredispersity)
            kptscores.append(kptscore)
            kpt_probs.append(kpt_prob)

        return keypoints, scoredispersitys, kptscores, kpt_probs
